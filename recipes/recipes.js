...
function search() {
    let recipeQuery = document.querySelector('#search-input').value;
...
function tagTemplate(tags) {
    return tags.map((tag)=> `<p class="recipe-tag">${tag}</p>`).join('');
}
...
function recipesTemplate(recipe){
    return `<div class="recipe-card">
            ...
            <div class="recipe-image">
                <img src="${recipe.image}" alt="A image depicting the recipe's description">
            </div>
        </div>`
}
